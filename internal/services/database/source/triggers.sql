...
BEGIN
    INSERT INTO HISTORICO_PLANO_DE_ASSINATURA(_ROWID, ID, NOME, VALOR_MENSAL, LIMITE_LINKS_MENSAL, LIMITE_REDIRECIONAMENTOS_MENSAL, ORDENACAO_ALEATORIA_LINKS, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (NEW.ROWID, NEW.ID, NEW.NOME, NEW.VALOR_MENSAL, NEW.LIMITE_LINKS_MENSAL, NEW.LIMITE_REDIRECIONAMENTOS_MENSAL, NEW.ORDENACAO_ALEATORIA_LINKS, NEW.CRIADO_EM, NEW.ATUALIZADO_EM, NEW.REMOVIDO_EM, 1, 511);
END;
...
BEGIN
    ...
    SELECT OLD.ROWID,
           ...
           CASE WHEN OLD.LIMITE_REDIRECIONAMENTOS_MENSAL != NEW.LIMITE_REDIRECIONAMENTOS_MENSAL THEN NEW.LIMITE_REDIRECIONAMENTOS_MENSAL ELSE NULL END,
           ...
           (CASE WHEN OLD.LIMITE_REDIRECIONAMENTOS_MENSAL != NEW.LIMITE_REDIRECIONAMENTOS_MENSAL THEN 16 ELSE 0 END) +
           (CASE WHEN OLD.ORDENACAO_ALEATORIA_LINKS != NEW.ORDENACAO_ALEATORIA_LINKS THEN 32 ELSE 0 END) +
...
BEGIN
    INSERT INTO HISTORICO_PLANO_DE_ASSINATURA(_ROWID, ID, NOME, VALOR_MENSAL, LIMITE_LINKS_MENSAL, LIMITE_REDIRECIONAMENTOS_MENSAL, ORDENACAO_ALEATORIA_LINKS, PERIODO_LIMITE, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (OLD.ROWID, OLD.ID, OLD.NOME, OLD.VALOR_MENSAL, OLD.LIMITE_LINKS_MENSAL, OLD.LIMITE_REDIRECIONAMENTOS_MENSAL, OLD.ORDENACAO_ALEATORIA_LINKS, OLD.CRIADO_EM, OLD.ATUALIZADO_EM, OLD.REMOVIDO_EM, (SELECT COALESCE(MAX(VERSAO), 0) FROM HISTORICO_PLANO_DE_ASSINATURA WHERE _ROWID = OLD.ROWID) + 1, -1);
END;
