...
BEGIN
    INSERT INTO HISTORICO_PLANO_DE_ASSINATURA(_ROWID, ID, NOME, VALOR, LIMITE_LINKS, ORDENACAO_ALEATORIA_LINKS, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (NEW.ROWID, NEW.ID, NEW.NOME, NEW.VALOR, NEW.LIMITE_LINKS, NEW.ORDENACAO_ALEATORIA_LINKS, NEW.CRIADO_EM, NEW.ATUALIZADO_EM, NEW.REMOVIDO_EM, 1, 255);
END;
...
BEGIN
    ...
    SELECT OLD.ROWID,
           ...
           CASE WHEN OLD.LIMITE_LINKS != NEW.LIMITE_LINKS THEN NEW.LIMITE_LINKS ELSE NULL END,
           ...
           (CASE WHEN OLD.LIMITE_LINKS != NEW.LIMITE_LINKS THEN 8 ELSE 0 END) +
...
BEGIN
    INSERT INTO HISTORICO_PLANO_DE_ASSINATURA(_ROWID, ID, NOME, VALOR, LIMITE_LINKS, ORDENACAO_ALEATORIA_LINKS, PERIODO_LIMITE, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (OLD.ROWID, OLD.ID, OLD.NOME, OLD.VALOR, OLD.LIMITE_LINKS, OLD.ORDENACAO_ALEATORIA_LINKS, OLD.CRIADO_EM, OLD.ATUALIZADO_EM, OLD.REMOVIDO_EM, (SELECT COALESCE(MAX(VERSAO), 0) FROM HISTORICO_PLANO_DE_ASSINATURA WHERE _ROWID = OLD.ROWID) + 1, -1);
END;
