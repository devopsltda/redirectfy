...
BEGIN
    INSERT INTO HISTORICO_LINK(_ROWID, ID, LINK, PLATAFORMA, REDIRECIONADOR, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (NEW.ROWID, NEW.ID, NEW.LINK, NEW.PLATAFORMA, NEW.REDIRECIONADOR, NEW.CRIADO_EM, NEW.ATUALIZADO_EM, NEW.REMOVIDO_EM, 1, 127);
END;
...
BEGIN
    ...
    SELECT OLD.ROWID,
           ...
           (CASE WHEN OLD.ID != NEW.ID THEN 1 ELSE 0 END) +
           (CASE WHEN OLD.LINK != NEW.LINK THEN 2 ELSE 0 END) +
           (CASE WHEN OLD.PLATAFORMA != NEW.PLATAFORMA THEN 4 ELSE 0 END) +
           (CASE WHEN OLD.REDIRECIONADOR != NEW.REDIRECIONADOR THEN 8 ELSE 0 END) +
           (CASE WHEN OLD.CRIADO_EM != NEW.CRIADO_EM THEN 16 ELSE 0 END) +
           (CASE WHEN OLD.ATUALIZADO_EM != NEW.ATUALIZADO_EM THEN 32 ELSE 0 END) +
...
BEGIN
    INSERT INTO HISTORICO_LINK(_ROWID, ID, LINK, PLATAFORMA, REDIRECIONADOR, CRIADO_EM, ATUALIZADO_EM, REMOVIDO_EM, VERSAO, BITMASK)
    VALUES (OLD.ROWID, OLD.ID, OLD.LINK, OLD.PLATAFORMA, OLD.REDIRECIONADOR, OLD.CRIADO_EM, OLD.ATUALIZADO_EM, OLD.REMOVIDO_EM, (SELECT COALESCE(MAX(VERSAO), 0) FROM HISTORICO_LINK WHERE _ROWID = OLD.ROWID) + 1, -1);
END;
