...
export class FormEditRedirectComponent implements OnInit {
  ...
  constructor(
    ...
    this.telegramForm = this.formBuilder.group({
      link: [
        '',
        [Validators.required, Validators.pattern(/^https:\/\/t\.me\/.*/)],
      ],
      nome: ['', [Validators.required]],
      plataforma: [],
      id: [],
  ...
  async ngOnInit() {
    ...
    this.prioridade =
      this.redirectData.redirecionador.ordem_de_redirecionamento;
  }
  ...
  async trocarPrioridade() {
    if (this.prioridade == 'whatsapp,telegram') {
      try {
        const res = await this.api.updateRedirect(this.redirectHash, {
      ...
      try {
        const res = await this.api.updateRedirect(this.redirectHash, {
          ordem_de_redirecionamento: 'whatsapp,telegram',
        });
        if (res.status == 200) {
    ...
    if (this.disableEditNome == true) {
      ...
      try {
        ...
        this.messageService.add({
          summary: 'Falha na Ação',
          detail:
            'Ocorreu um erro ao renomear o redirecionador, ação não executada',
          severity: 'error',
        });
      }
  ...
  async saveEdits(plataforma: string) {
    ...
    if (plataforma == 'whatsapp') {
      if (this.whatsappForm.valid) {
        ...
        try {
          await this.api.updateLinkInRedirect(
            this.redirectHash,
            editedData['id'],
            submitEditData
          );
          ...
          this.messageService.add({
            summary: 'Falha ao Editar Redirecionador',
            detail:
              'Ocorreu um erro ao editar o redirecionador, ação não executada',
            severity: 'error',
          });
        }
        this.whatsappForm.reset();
      ...
      if (this.telegramForm.valid) {
        ...
        editedData['plataforma'] = 'telegram';
        try {
          await this.api.updateLinkInRedirect(
            this.redirectHash,
            editedData['id'],
  ...
  async onSubmit() {
    ...
    try {
      const resApi = await this.api.addLinkToRedirect(
        this.redirectHash,
        this.submitData
      );
      ...
      this.messageService.add({
        summary: 'Falha ao Criar Redirecionador',
        detail: 'Ocorreu um erro ao criar o redirecionador, ação não executada',
        severity: 'error',
      });
    }
  }
